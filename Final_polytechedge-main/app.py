import re
from flask import Flask, jsonify, render_template, request, redirect, send_from_directory, session, url_for, flash, Response, current_app, abort
import os
import json
...
if _db_uri and _db_uri.startswith('postgres://'):
    _db_uri = _db_uri.replace('postgres://', 'postgresql://', 1)  # SQLAlchemy 1.4+ expects postgresql://
app.config['SQLALCHEMY_DATABASE_URI'] = _db_uri or 'sqlite:///polytechedge.db'
...
def favicon():
    ...
    if os.path.exists(fav_path) and os.path.isfile(fav_path):
        return send_from_directory(static_dir, 'favicon.ico')
    # Avoid crashing — return No Content if missing
...
def google_login():
    ...
    if not app.config.get('GOOGLE_CLIENT_ID') or not app.config.get('GOOGLE_CLIENT_SECRET'):
        flash('Google Sign-In is not configured. Please contact the administrator.', 'warning')
...
@app.route('/polyadmission')
def polyadmission():
    return render_template('/polyadmission.html')


@app.route('/dseadmission')
def dseadmission():
    return render_template('/dseadmission.html')
...
def to_list(value):
    """Convert string or list to list, handling comma-separated strings"""
...
def recommend():
    try:
        ...
        branches = to_list(data.get("branches") or data.get("branch", []))
        interests = to_list(data.get("interests", []))
...
def guidance_job_results():
    ...
    return redirect(url_for('guidance_job_results'))

@app.route('/coming-soon')
def coming_soon():
    """Route for the Coming Soon page for upcoming career guidance features"""
...
# Register a Jinja test named 'ge' for use with selectattr
@app.template_test('ge')
def ge_test(value, other):
    try:
        return value >= other
    except Exception:
        return False
...
def load_model_and_data():
    ...
    processed_path = os.path.join('data', 'processed_cutoff_data.csv')
    cleaned_path = os.path.join('data', 'cleaned_cutoff_data.csv')

    if not os.path.exists(model_path) or not os.path.exists(encoder_path):
        print("Model or encoder file not found. Please run train_model.py first.")
        return None, None, None

    # Prefer processed cutoff data (has city and round columns)
    data_path = None
...
def step2():
    if request.method == 'POST':
        ...
        # Persist selected category for results header
        ...
        # Map to the keys expected by utils/prediction.py
        user_input = {
            'percentage': marks,      # prediction.py expects 'percentage'
            'category': category,
            'branch': branch,
            'college_type': college_type,
            'city': location          # prediction.py expects 'city'
        }
        ...
        try:
            ...
            err = str(e)
            print("[Predictor] Error:", err)
            print(traceback.format_exc())
            return render_template('error.html', message=f"Prediction failed: {err}")

        # Store recommendations in session
        recs = recommendations.to_dict('records')

        # Augment records so the results template has all expected fields
        user_marks_val = session.get('user_marks', 0)
        user_cat_val = (session.get('user_category') or 'GENERAL').upper()
        for rec in recs:
            # Map city -> location for template compatibility
            rec['location'] = rec.get('city', '')
            # Predicted cutoff may not be present; default to adjusted marks to avoid errors
            adj = float(rec.get('adjusted_percentage', 0) or 0)
            pred = rec.get('predicted_cutoff')
            try:
                pred_val = float(pred) if pred is not None else adj
            except Exception:
                pred_val = adj
            rec['predicted_cutoff'] = round(pred_val, 2)
            # Difference between adjusted marks and predicted cutoff
            rec['mark_difference'] = round(adj - pred_val, 2)
            # Trend info defaults
            rec['cutoff_trend'] = rec.get('cutoff_trend', 'stable')
            rec['trend_percentage'] = rec.get('trend_percentage', 0)
            # Year (use latest known or default)
            rec['year'] = rec.get('year', '2024')
            # College URL via helper
            rec['college_url'] = get_college_url(rec.get('college_name', ''))
            # Profile summary fields
            rec['user_marks'] = user_marks_val
            rec['adjusted_marks'] = adj
            rec['user_category'] = user_cat_val
...
# --------------------------------------------------
# Global error handlers to avoid generic 500 pages
# --------------------------------------------------
...
def serve_static(filename):
    # Ignore Vite dev client path which is not served by Flask
    if filename.startswith('@vite'):
        return abort(404)

    # Serve common static assets directly
    if filename.endswith(('.jpg', '.jpeg', '.png', '.svg', '.webp', '.ico', '.css', '.js')) or filename.startswith('static/'):
        return send_from_directory('.', filename)

    # Only render HTML templates by filename
    ...
    # Unknown path — do not try to render as a template
    return abort(404)
...
def admin():
    ...
    if request.method == 'POST':
        ...
        if admin_password == 'p_olytechEDGE2025':
